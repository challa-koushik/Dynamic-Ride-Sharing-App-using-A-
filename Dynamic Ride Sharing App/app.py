...
def accept_ride(ride_id):
    ...
    try:
        ...
        return render_template('single_ride_map.html',
                             ride=dict(ride),
                             rider_location={'lat': rider_lat, 'lon': rider_lon},
                             rider_city=rider_city,
                             rider_to_pickup_path=rider_to_pickup_path,
                             rider_to_pickup_coords=rider_to_pickup_coords,
                             rider_to_pickup_distance=round(rider_to_pickup_distance, 2),
                             user_path=user_path,
...
def show_route(ride_id):
    """Show route with rider location if ride is accepted"""
    if 'user_id' not in session:
        return redirect(url_for('login'))
    ...
    ride = conn.execute('''
        ...
        FROM rides
        LEFT JOIN users as riders ON rides.rider_id = riders.id
        WHERE rides.id = ? AND rides.user_id = ?
    ...
    if ride['status'] == 'accepted' and ride['rider_lat'] and ride['rider_lon']:
        rider_data = {
            ...
            'lat': ride['rider_lat'],
            'lon': ride['rider_lon'],
            'distance_to_pickup': round(calculate_distance(
                ride['rider_lat'], ride['rider_lon'],
                ride['pickup_lat'], ride['pickup_lon']
            ), 2)
        }
